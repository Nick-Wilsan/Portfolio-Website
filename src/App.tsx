...
const SectionLabel = ({ children }: { children: string }) => (
  <p className="text-xs font-semibold tracking-[0.08em] uppercase text-zinc-500 mb-2">{children}</p>
);
...
const Hero = () => (
  <section id="hero" className="max-w-[1440px] mx-auto px-5 md:px-14 pt-36 md:pt-44 pb-[120px]">
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] xl:grid-cols-[1fr_420px] gap-12 xl:gap-16 items-center">
      ...
      <div className="flex flex-col gap-8">
        ...
        >
          <a href="#projects" className="flex items-center gap-2 bg-zinc-900 text-white px-8 py-4 rounded-full text-base font-semibold hover:bg-emerald-500 transition-colors shadow-sm">
            View Portfolio <IconArrow />
          </a>
          <a href="#contact" className="flex items-center gap-2 border border-zinc-200 text-zinc-900 px-8 py-4 rounded-full text-base font-semibold hover:border-emerald-500 hover:text-emerald-500 transition-colors">
      ...
      >
        {/* Stats grid */}
        <div className="bg-slate-50 rounded-2xl border border-zinc-200 p-6">
          <p className="text-[11px] font-semibold tracking-[0.1em] uppercase text-zinc-400 mb-5">FROM THE WORK</p>
          <div className="grid grid-cols-2 gap-3">
            {heroStats.map((s, i) => (
              <motion.div
                ...
                className="bg-white rounded-xl border border-zinc-200 p-4 hover:border-emerald-500 transition-colors"
              >
                <p className="font-display text-2xl font-bold text-zinc-900">{s.num}</p>
        ...
        <div className="bg-zinc-900 rounded-2xl p-5">
          <p className="text-[11px] font-semibold tracking-[0.1em] uppercase text-white/40 mb-3">TOOLKIT</p>
          <div className="flex flex-wrap gap-2">
            {["PRD Writing", "User Research", "RICE", "AI Workflow", "Go-to-Market", "Figma"].map((s) => (
              <span key={s} className="text-xs font-semibold bg-white/10 text-white/80 px-3 py-1.5 rounded-full border border-white/10 hover:bg-emerald-500/20 hover:border-emerald-500/40 transition-colors">
                {s}
              </span>
            ))}
          </div>
  ...
  <section id="projects" className="py-[120px] max-w-[1440px] mx-auto px-5 md:px-14">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <motion.article
        ...
        viewport={{ once: true }}
        transition={{ duration: 0.6 }}
        className="bg-slate-50 rounded-xl border border-zinc-200 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group"
      >
          ...
          {experiences.map((exp, i) => (
            ...
            >
              <div className={`absolute -left-[41px] md:-left-[57px] top-1.5 w-4 h-4 rounded-full z-10 ${
                exp.active ? "bg-emerald-500 border-2 border-emerald-500" : "bg-white border-2 border-zinc-300"
              }`} />
              <p className="text-xs font-semibold tracking-[0.05em] uppercase text-zinc-500 mb-2">{exp.period}</p>
              <h3 className="font-display text-[1.5rem] font-semibold text-zinc-900 mb-1">{exp.role}</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {groups.map((group, i) => (
          <motion.div
            ...
            className="bg-slate-50 rounded-xl border border-zinc-200 p-6 hover:border-zinc-300 hover:shadow-md transition-all"
          >
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-lg bg-white border border-zinc-200 flex items-center justify-center text-zinc-900">
                {group.icon}
              </div>
              <h3 className="font-display text-[1.25rem] font-semibold text-zinc-900">{group.title}</h3>
            </div>
            <div className="flex flex-wrap gap-2">
              {group.skills.map((s) => (
                <span key={s} className={`text-[11px] font-semibold tracking-wide uppercase px-3 py-1.5 rounded-full border ${group.color}`}>
                  {s}
                </span>
              ))}
            </div>
        ...
        <div className="flex flex-col gap-6">
          {[
            ...
            <div key={item.label} className="flex items-center gap-4 group">
              <div className="w-12 h-12 rounded-full bg-white border border-zinc-200 flex items-center justify-center text-zinc-900 group-hover:bg-emerald-500 group-hover:border-emerald-500 group-hover:text-white transition-all shrink-0">
                {item.icon}
              </div>
              <div>
                <p className="text-xs font-semibold tracking-[0.08em] uppercase text-zinc-400 mb-0.5">{item.label}</p>
